...
class EEP:
    ...
    def __init__(self, filename, verbose=True):
        ...
        self.version, self.abun, self.rot, self.minit, self.hdr_list, self.eeps = (
            self.read_eep_file()
        )
...
class EEPCMD:
    ...
    def plot_CMD(self, filters, fignum=0, phases=[], phasecolor=[], **kwargs):
        ...
        fig = plt.figure(fignum)
        plt.xlabel(
            " ".join(filters[0].split("_")) + "-" + " ".join(filters[1].split("_")),
            fontsize=22,
        )
        ...
        if len(phases) >= 0:
            ...
            for i_p, phase in enumerate(phases):
                ...
                if len(p_ind) > 0:
                    if phasecolor == "":
                        ax.plot(
                            x1[p_ind] - x2[p_ind], y[p_ind], linewidth=4.0, alpha=0.5
                        )
                    else:
                        ax.plot(
                            x1[p_ind] - x2[p_ind],
