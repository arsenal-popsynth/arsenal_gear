...
def mdot_vink_ostar_useGamma(Mstar, Age, iso):
    ...
    term2_hi = (
        0.933 * (logTeff - np.log10(4e4))
        - 10.92 * ((logTeff - np.log10(4e4)) ** 2)
        + 0.85 * (logZ - logZsun)
    )
...
def mdot_VS21_ostar(Mstar, Age, iso, use2001=False):
    ...
    T1 = (61.2 + (2.59 * charrho)) * 1000.0
    T2 = (100.0 + (6.0 * charrho)) * 1000.0

    Teff = 10**logTeff
    # get ratio of wind-velocity to escape velocity
    ratio = 2.6 * (Teff > T1) + 0.7 * (Teff < T2) + 1.3 * (Teff > T2) * (Teff < T1)
    offset = (
        -6.697 * (Teff > T1) + -5.99 * (Teff < T2) + -6.688 * (Teff > T2) * (Teff < T1)
    )

    lograt = np.log10(ratio / 2.0)
...
def mdot_SV20_WR(Mstar, Age, iso):
    ...
    result = (
        2.932 * np.log10(-1 * np.log10(1 - Gamma))
        - np.log10(2) * ((Gammaeb / Gamma) ** c)
        + logMdot_off
    )
